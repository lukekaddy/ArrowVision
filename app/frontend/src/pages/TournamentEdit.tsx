...
export default function TournamentEdit() {
  ...
  const [notUpcoming, setNotUpcoming] = useState(false);
  ...
  const fetchTournament = async () => {
    ...
    try {
      ...
      // Divisions
      if (data.divisions) {
        const divs = data.divisions.split(',').map((d: string) => d.trim()).filter(Boolean);
      ...
      if (data.courses) {
        try {
          const parsedCourses = JSON.parse(data.courses);
      ...
      if (data.scoring_template_id) {
        setSelectedScorecardId(data.scoring_template_id.toString());
      }

      // Mulligans
      if (data.mulligans) {
        try {
          const mulliganConfig: MulliganConfig = typeof data.mulligans === 'string'
            ? JSON.parse(data.mulligans)
            : data.mulligans;
          if (mulliganConfig.enabled) {
            setMulligansEnabled(true);
            ...
            mulliganConfig.types.forEach((mt) => {
              // Map back to option key
              const key = MULLIGAN_TYPE_OPTIONS.includes(mt.name) ? mt.name : 'Custom';
              if (key === 'Custom') {
                setCustomMulliganName(mt.name);
              }
              typeNames.push(key);
              maxAllowed[key] = mt.maxAllowed;
              if (mt.restrictedTargets && mt.restrictedTargets.length > 0) {
                restricted[key] = true;
                restrictedTargets[key] = mt.restrictedTargets.join(', ');
              }
            });
            setSelectedMulliganTypes(typeNames);
            setMulliganMaxAllowed(maxAllowed);
            setMulliganRestricted(restricted);
            setMulliganRestrictedTargets(restrictedTargets);
          }
        } catch {
          // keep defaults
        }
